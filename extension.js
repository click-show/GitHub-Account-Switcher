...
async function activate(ctx) {
    const disposables = [];
    let accountManagerSidebarView;
    disposables.push(vscode.window.registerWebviewViewProvider(ACCOUNT_MANAGER_VIEW_ID, {
    ...
    const addAccountHandler = async (prefilledLabel) => {
        try {
            ...
            let session;
            try {
                session = await vscode.authentication.getSession('github', scopes, { forceNewSession: { detail: 'Sign in with the GitHub account you want to add to GHAS.' }, silent: false });
            }
            catch {
                // forceNewSession may throw in some VS Code versions/environments; fall back gracefully
                session = await vscode.authentication.getSession('github', scopes, { createIfNone: true, silent: false });
            }
            if (!session)
                return;
            const accounts = await getSavedAccounts(ctx);
            // Check for duplicate account (same GitHub username/label)
            const duplicate = accounts.find(a => isSameGithubAccount(a.accountLabel, session.account.label) &&
                a.label.trim().toLowerCase() === label.trim().toLowerCase());
            if (duplicate) {
                const action = await vscode.window.showWarningMessage(`An account already exists with label "${label}" and GitHub user "${session.account.label}". Overwrite the token?`, 'Overwrite', 'Cancel');
                if (action !== 'Overwrite')
                    return;
    ...
    const switchAccountHandler = async (accountId) => {
        try {
            const config = vscode.workspace.getConfiguration();
    ...
    const refreshAccountHandler = async (accountId) => {
        try {
            const config = vscode.workspace.getConfiguration();
            const scopes = getScopes(config);
            const ghasBin = getGhasBin(config);
            ...
            if (!isSameGithubAccount(session.account.label, chosen.accountLabel)) {
                vscode.window.showErrorMessage(`Selected sign-in account (${session.account.label}) does not match "${chosen.accountLabel}". Refresh cancelled.`);
                return;
            }
    ...
    const deleteAccountHandler = async (accountId, skipConfirmation = false) => {
        try {
            const accounts = await getSavedAccounts(ctx);
            if (!accounts.length) {
            ...
            if (!chosen)
                return;
            if (!skipConfirmation) {
                const confirmation = await vscode.window.showWarningMessage(`Delete account "${formatAccount(chosen)}" from GHAS saved accounts?`, { modal: true }, 'Delete');
    ...
    const cloneWithAccountHandler = async (accountId) => {
        try {
            const config = vscode.workspace.getConfiguration();
            const ghasBin = getGhasBin(config);
            if (!isGithubAuthenticationAvailable()) {
            ...
            let repos;
            await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Loading repos for ${chosen.accountLabel}…`, cancellable: false }, async () => {
...
function isGithubAuthenticationAvailable() {
    return typeof vscode.authentication.getSession === 'function';
}
function getScopes(config) {
    return config.get(GHAS_SCOPES_KEY)
        ?? ['repo', 'read:org'];
}
function getGhasBin(config) {
    const configuredBin = (config.get(GHAS_BIN_KEY) ?? 'gh').trim();
    if (!configuredBin) {
        return 'gh';
    }
...
async function pickAccount(accounts, placeHolder) {
    const pick = await vscode.window.showQuickPick(accounts.map(formatAccount), { placeHolder });
    if (!pick)
        return undefined;
 ...
 */
async function logoutFromGhas(ghasBin) {
    try {
...
async function loginToGhas(ghasBin, token) {
 ...
 */
async function setupGitAuthForGhas(ghasBin) {
    await runGhasCommand(ghasBin, ['auth', 'setup-git']);
}
async function runGhasCommand(ghasBin, args, stdinData) {
    const preferredBins = (0, ghasCli_1.resolvePreferredBins)(ghasBin);
    const launchCandidates = (0, ghasCli_1.buildLaunchCandidates)(preferredBins);
    const attemptedBins = [];
    ...
    throw new Error(`GHAS CLI not found. Tried: ${attemptedBins.map(bin => `"${bin}"`).join(', ')}. Set "ghas.ghasPath" to your GHAS executable (for example: C:\\Tools\\ghas.exe) or "gh", then reload VS Code.`);
}
async function runSingleCliCommand(bin, args, stdinData) {
...
function isSameGithubAccount(current, expected) {
    return current.trim().toLowerCase() === expected.trim().toLowerCase();
}
async function revealAccountManagerView(sidebarView) {
...
async function initializeAccountManagerWebview(webview, ctx) {
    webview.options = {
        enableScripts: true
    };
        ...
        try {
            switch (message.type) {
                case 'refreshList':
                    ...
                    return;
                case 'add': {
                    const result = await vscode.commands.executeCommand(COMMANDS.addAccount, message.label);
                ...
                case 'switch': {
                    ...
                    const result = await vscode.commands.executeCommand(COMMANDS.switchAccount, accountId);
                ...
                case 'refreshToken': {
                    ...
                    const result = await vscode.commands.executeCommand(COMMANDS.refreshAccount, accountId);
                ...
                case 'delete': {
                    ...
                    const result = await vscode.commands.executeCommand(COMMANDS.deleteAccount, accountId, true);
                ...
                case 'clone': {
                    ...
                    const result = await vscode.commands.executeCommand(COMMANDS.cloneWithAccount, accountId);
...
function getActiveAccountId(ctx) {
    return ctx.globalState.get(ACTIVE_ACCOUNT_STATE_KEY);
}
...
function getAccountManagerHtml(webview, accounts, activeAccountId, notice) {
...
function describeTokenState(tokenExpiry) {
