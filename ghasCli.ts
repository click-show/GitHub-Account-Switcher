...
function isSimpleCommandName(value: string): boolean {
  return !/[\\/]/.test(value);
}

function hasExecutableExtension(value: string): boolean {
  return /\.[^./\\]+$/.test(value);
}
...
export function resolvePreferredBins(configuredBin: string | undefined | null): string[] {
...
function collectUserProfileRoots(env: NodeJS.ProcessEnv): string[] {
  const roots: string[] = [];
...
function collectLocalAppDataRoots(env: NodeJS.ProcessEnv): string[] {
  const roots: string[] = [];
  if (env.LOCALAPPDATA) {
...
function getPathVariable(env: NodeJS.ProcessEnv): string | undefined {
  return env.PATH ?? env.Path ?? env.path;
}

function parseWindowsPathExt(env: NodeJS.ProcessEnv): string[] {
...
function buildPathDerivedCandidates(
  bin: string,
...
function buildWindowsGhFallbacks(bin: string, env: NodeJS.ProcessEnv): string[] {
...
function buildPosixGhFallbacks(bin: string, platform: NodeJS.Platform): string[] {
  const lower = bin.toLowerCase();
